...
class DataTaskHandler extends BinaryHttpResponseHandler {
	...
	public void onFailure(int i,
			cz.msebera.android.httpclient.Header[] headers, byte[] errorResponse,
			Throwable throwable) {
		 String errStr = "";
	        if (null != throwable) {
        ...
        Runnable taskRunnable = new Runnable() {
            ...
            public void run() {
                ...
                do {
                    ...
                    cz.msebera.android.httpclient.Header[] headers = null;
                    long fileLen = tempFile.length();
                    if (fileLen > 0) {
                        // continue download
                        List<Header> list = new ArrayList<Header>();
                        list.add(new BasicHeader("Range", "bytes=" + fileLen + "-"));
                        headers = list.toArray(new cz.msebera.android.httpclient.Header[list.size()]);
                    }
